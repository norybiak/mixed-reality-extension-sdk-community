	...
	'actor-correction': {
		...
		session: {
			...
			beforeReceiveFromClient: (
				...
				const syncActorParent = session.actorSet.get(syncActor.initialization.message.payload.actor.parentId);
				const attached = syncActor?.initialization.message.payload.actor.attachment;
				const parentAttached = syncActorParent?.initialization.message.payload.actor.attachment;
				if (syncActor && ((client.authoritative && !syncActor.grabbedBy) ||
					(syncActor.grabbedBy === client.id) ||
                    (attached && attached.userId === client.userId) ||
					(parentAttached && parentAttached.userId === client.userId))) {
